...
import { productInsertSchema, ProductInsertType } from "@/db/schema/product";
...
export default function EditProductForm({
	...
	const [selectedImages, setSelectedImages] = useState<string[]>(() =>
		product.image.map((img: ImageInsertType) => img.imageUrl)
	);
	...
	function onSubmit(values: z.infer<typeof productInsertSchema>) {
		startTransition(async () => {
			...
			await updateProduct(values, urls, productId).then((data) => {
				form.reset();
			});
